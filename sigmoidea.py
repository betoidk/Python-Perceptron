...
for epoch in range(epochs):  # Para cada epoca se itera sobre cada ejemplo
    ...
    # Para cada ejemplo i=1, 2, 3, 4 se obtiene el vector de características xi y la etiqueta yi
    for xi, yi in zip(Xb, y):
        # Calcular potencial de activación Z = w1*x1 + w2+x2 + b
        z = np.dot(xi, w)
        # Calcular salida del perceptrón yout = f(z)
        yout = 1 if sigmoidea(z) >= 0.5 else 0
...
def predict(x):
    ...
    # Devolver la prediciión usando la función de activación
    return 1 if sigmoidea(z) >= 0.5 else 0
...
y_pred = np.array([1 if sigmoidea(np.dot(xi, w)) >= 0.5 else 0 for xi in Xb])
